...
export default function App(){
  ...
  return<div className="flex flex-col" style={{background:"#04070D",minHeight:"100vh"}}>
    ...
    {alerts.length>0&&<div aria-live="assertive" className="flex-shrink-0 px-4 py-1 flex items-center gap-2" style={{borderBottom:"1px solid #1A2744"}}><span className="mono text-[9px] font-bold flex-shrink-0" style={{color:"#ECC94B"}}>⚡</span><div className="flex-1 overflow-hidden whitespace-nowrap"><div className="inline-block" style={{animation:"tk 25s linear infinite"}}>{[...alerts,...alerts].map((a,i)=><span key={i} className="mono text-[10px] mx-5" style={{color:a.s==="h"?"#E53E3E":"#7A879C"}}>{a.s==="h"?"🔴":"🔵"} {a.t}</span>)}</div></div></div>}

    <div className="flex-1 overflow-auto">
      {/* OPERATIONAL STATUS — real metrics that matter */}
      <div className="p-3 pb-2"><div className="pn overflow-hidden" style={{borderTop:"2px solid #5B9CF6"}}>
        <div className="flex items-center justify-between px-4 py-1.5" style={{borderBottom:"1px solid #1A2744"}}><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{background:"#5B9CF6",animation:"lp 1.5s infinite"}}/><span className="text-sm font-bold" style={{color:"#5B9CF6"}}>OPERATIONAL STATUS</span><FreshnessIndicator lastUpdated={lastFetchDate}/></div><span className="mono text-[8px] px-2 py-0.5 rounded" style={{background:"rgba(91,156,246,.06)",color:"#5B9CF6",border:"1px solid rgba(91,156,246,.1)"}}>LIVE</span></div>
      ...
      <div className="px-3 pb-3 flex gap-2" style={{minHeight:400}}>
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex items-center justify-between mb-1"><span className="mono text-[9px] font-bold text-gray-500 tracking-widest flex items-center gap-1">ACTIVE INCIDENTS<FreshnessIndicator lastUpdated={lastFetchDate}/></span></div>
          <div className="flex gap-1 mb-1.5 flex-wrap">{(["all","ems","fire","tc","hazmat","rescue"] as const).map(f=>{const cnt=f==="all"?incidents.length:st.c[f];const act=flt===f;return<button key={f} onClick={()=>setFlt(f)} className={`px-2 py-0.5 rounded mono text-[9px] font-bold ${act?"text-white":"text-gray-500"}`} style={{background:act?`${f==="all"?"#5B9CF6":CATEGORY_COLORS[f as IncidentCategory]}15`:"#0E1525",border:`1px solid ${act?`${f==="all"?"#5B9CF6":CATEGORY_COLORS[f as IncidentCategory]}25`:"#1A2744"}`}}>{f==="all"?"ALL":CATEGORY_LABELS[f as IncidentCategory]}{cnt>0&&<span className="ml-1 text-gray-600">{cnt}</span>}</button>;})}</div>
          <div aria-live="polite" className="flex-1 overflow-auto space-y-1">{fltd.map((inc,i)=>{const cat=getCategory(inc.type),ms=getMostActiveStatus(inc.units),sc2=STATUS_COLORS[ms]||"#3D4D66",el=elapsedMinutes(inc.time),hv=inc.units.length>=6;return<button type="button" key={inc.id||i} tabIndex={i} onClick={()=>setSel(inc)} className="ir pn rounded-lg px-3 py-2.5 flex items-center gap-3 fi w-full text-left focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-400" style={{animationDelay:`${Math.min(i*15,150)}ms`,borderLeftColor:CATEGORY_COLORS[cat]}}><div className="w-11 text-center flex-shrink-0"><div className="mono text-sm font-bold text-gray-300">{formatTime(inc.time)}</div><div className="mono text-[8px] text-gray-600">{el}m</div></div><span className={`flex-shrink-0 px-2 py-0.5 rounded mono text-[10px] font-bold c${cat[0]}`}>{TYPE_SHORT[inc.type]||inc.type}</span><div className="flex-1 min-w-0"><div className={`text-sm truncate ${hv?"text-white font-semibold":"text-gray-200"}`}>{formatAddress(inc.addr)}</div><div className="flex gap-1 mt-0.5">{inc.units.slice(0,4).map((u,j)=><span key={u.id+j} className="mono text-[9px] font-bold px-1 rounded" style={{color:STATUS_COLORS[u.status]||"#3D4D66",background:`${STATUS_COLORS[u.status]||"#3D4D66"}0D`}}>{u.id}</span>)}{inc.units.length>4&&<span className="mono text-[9px] text-gray-600">+{inc.units.length-4}</span>}</div></div><div className="flex-shrink-0 text-right"><div className="mono text-[11px] font-bold" style={{color:sc2}}>{STATUS_LABELS[ms]||ms}</div><div className="mono text-[9px] text-gray-500">{inc.units.length}u</div></div></button>;})}
