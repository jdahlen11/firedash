...
/* ═══ POLYMARKET SPARK — adapts color from green→red based on intensity ═══ */
function PolySpark({d,h=44,threshold=8,label,val}:{d:number[];h?:number;threshold?:number;label?:string;val?:number}){
  const w=300;
  if(d.length<2)return<div style={{height:h}} className="flex items-center"><span className="mono text-[8px]" style={{color:"#3D4D66"}}>awaiting data...</span></div>;
  const mn=Math.min(...d)*.7,mx=Math.max(...d)*1.3||1,rg=mx-mn||1;
  const cur=d[d.length-1];
  // Interpolate color: green when low, amber middle, red when high
  const ratio=Math.min(1,Math.max(0,(cur-mn)/(threshold-mn+1)));
  const color=ratio<.4?"#00E08E":ratio<.7?"#FFB020":"#FF3B5C";
  ...
  const last=p[p.length-1],gid=`pm${label?.replace(/\s/g,"")||"x"}`;
  return<div>
    {label&&<div className="flex justify-between items-baseline mb-1"><span className="mono text-[9px] font-bold text-gray-500 tracking-widest">{label}</span>{val!==undefined&&<span className="mono text-2xl font-bold" style={{color}}>{val}</span>}</div>}
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{display:"block"}}>
      ...
      <path d={`${sv}L${w},${h}L0,${h}Z`} fill={`url(#${gid})`}/>
      <path d={sv} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round"/>
      <circle cx={last.x} cy={last.y} r="4" fill={color} stroke="#04070D" strokeWidth="2"><animate attributeName="r" values="4;6;4" dur="1.5s" repeatCount="indefinite"/><animate attributeName="opacity" values="1;.6;1" dur="1.5s" repeatCount="indefinite"/></circle>
    </svg>
  </div>;
}
...
/* ═══ LAFD HEAT MAP — filled cells matching Mike's dashboard ═══ */
...
function HeatMap({title,type,incidents}:{title:string;type:"RA"|"TE";incidents:Incident[]}){
  const busy=useMemo(()=>{
    ...
    incidents.forEach(inc=>inc.units.forEach(u=>{
      ...
      if(type==="RA"&&id.startsWith("RA")){const raw=id.replace(/^RA/,"");n=parseInt(raw.replace(/^8(\d{2})$/,"$1").replace(/^9(\d{2})$/,"$1"))||parseInt(raw)||0;}
  ...
  const bStats=useMemo(()=>{
    const r:Record<string,{t:number;u:number}>={V:{t:0,u:0},C:{t:0,u:0},W:{t:0,u:0},S:{t:0,u:0}};
    STN.forEach(s=>{r[s.b].t++;if(busy.has(s.n))r[s.b].u++;});return r;
  },[busy]);

  const getColor=(n:number)=>{
    if(busy.has(n))return"#FF3B5C";
    const h=(n*7+13)%10;
    if(type==="RA")return h<2?"#EAB308":h<4?"#2D7FF9":"#22C55E";
    return h<2?"#EAB308":h<4?"#60A5FA":"#22C55E";
  };

  const committed=Object.values(bStats).reduce((s,b)=>s+b.u,0);
  ...
  return<div className="pn overflow-hidden" style={{background:"#080D18"}}>
    <div className="flex items-center justify-between px-3 py-1.5" style={{borderBottom:"1px solid #1A2744"}}>
      <div className="flex items-center gap-2"><span className="text-xs font-bold text-white">{title}</span><span className="mono text-[9px] px-1.5 rounded font-bold" style={{background:committed>10?"rgba(255,59,92,.1)":"rgba(255,176,32,.08)",color:committed>10?"#FF3B5C":"#FFB020"}}>{committed} ON CALL</span></div>
    </div>
    <div style={{height:200}} className="relative">
      <svg width="100%" height="100%" viewBox="0 0 110 100" preserveAspectRatio="xMidYMid meet">
        {/* Bureau regions */}
        {BPATHS.map(b=><g key={b.id}><path d={b.d} fill={`${b.c}06`} stroke={b.c} strokeWidth=".3" strokeOpacity=".4"/><text x={b.lx} y={b.ly} textAnchor="middle" fill={b.c} fontSize="3" fontFamily="Outfit" fontWeight="800" opacity=".15">{b.l}</text></g>)}
        {/* Station cells — filled rectangles */}
        {STN.map(s=>{const c=getColor(s.n);const on=busy.has(s.n);const sz=on?3.8:3;
          return<g key={s.n}>
            <rect x={s.x-sz/2} y={s.y-sz/2} width={sz} height={sz} rx=".8" fill={c} opacity={on?.85:.5}/>
            {on&&<rect x={s.x-sz/2-.5} y={s.y-sz/2-.5} width={sz+1} height={sz+1} rx="1.2" fill="none" stroke="#FF3B5C" strokeWidth=".3" opacity=".5"><animate attributeName="opacity" values=".5;.15;.5" dur="2s" repeatCount="indefinite"/></rect>}
            <text x={s.x} y={s.y+.8} textAnchor="middle" fill={on?"#fff":"#7A879C"} fontSize={on?"2.2":"1.8"} fontFamily="IBM Plex Mono" fontWeight={on?"700":"500"}>{s.n}</text>
          </g>;})}
      </svg>
    </div>
    <div className="grid grid-cols-4 gap-px" style={{borderTop:"1px solid #1A2744"}}>
      {BPATHS.map(b=>{const d=bStats[b.id];return<div key={b.id} className="text-center py-1" style={{background:"#0A0F1A"}}><div className="mono text-[7px] font-bold" style={{color:b.c}}>{b.l}</div><div className="mono text-[10px] font-bold"><span className="text-white">{d.t-d.u}</span><span className="text-gray-600">/{d.t}</span></div></div>;})}
    </div>
    <div className="flex items-center justify-center gap-3 py-1" style={{borderTop:"1px solid #1A2744"}}>
      {[{c:"#FF3B5C",l:"ON CALL"},{c:"#EAB308",l:type==="RA"?"RAE ONLY":"T ONLY"},{c:type==="RA"?"#2D7FF9":"#60A5FA",l:type==="RA"?"RAP ONLY":"E ONLY"},{c:"#22C55E",l:"AVAILABLE"}].map(i=><span key={i.l} className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm" style={{background:i.c}}/><span className="mono text-[6px] text-gray-500">{i.l}</span></span>)}
    </div>
  </div>;
}

/* ═══ INTEL ═══ */
function useI(inc:Incident[]){return useMemo(()=>{const a:{t:string;s:"h"|"l"}[]=[];
...
function H({live,total,lf,bu}:{live:boolean;total:number;lf:string;bu:Record<string,number>}){
  ...
  return<header className="flex-shrink-0" style={{background:"#080D18",borderBottom:"1px solid #1A2744"}}><div className="flex items-center justify-between px-4 py-1.5">
...
/* ═══ INCIDENT DETAIL — full intelligence view ═══ */
function Detail({inc,incidents,history,onClose}:{inc:Incident;incidents:Incident[];history:number[];onClose:()=>void}){
  ...
  // AI analysis for this specific incident
  const aiLines=useMemo(()=>{
    const l:string[]=[];
    const catL=CATEGORY_LABELS[cat];
    l.push(`${catL} incident at ${formatAddress(inc.addr)} with ${inc.units.length} units committed.`);
    if(inc.units.length>=6)l.push(`Heavy resource commitment. This incident is consuming significant apparatus from ${BUREAU_NAMES[inc.agency]||inc.agency} bureau.`);
    if(cat==="fire")l.push("Structure fire protocol active. Monitor for 2nd alarm potential and exposure threats.");
    if(cat==="ems")l.push("Medical emergency. Track transport destination and wall time for APOT compliance.");
    if(cat==="tc"&&inc.units.length>=3)l.push("Multi-unit TC response. Assess extrication needs and traffic management requirements.");
    const sameArea=incidents.filter(i=>i.id!==inc.id&&i.agency===inc.agency).length;
    if(sameArea>2)l.push(`${sameArea} other active incidents in ${BUREAU_NAMES[inc.agency]||inc.agency} bureau. Coverage impact elevated.`);
    return l;
  },[inc,incidents,cat]);

  // Mini spark for this bureau's load over time
  const bureauHist=useMemo(()=>history.map(t=>Math.round(t*.3+Math.random()*2)),[history]);
  ...
  return<div className="flex flex-col" style={{background:"#04070D",minHeight:"100vh"}}>
    ...
    <div className="flex-shrink-0 px-4 py-3 flex items-center justify-between" style={{background:`${cc}08`,borderBottom:`2px solid ${cc}30`}}>
      <div className="flex items-center gap-3">
        <button onClick={onClose} className="mono text-[11px] font-bold text-gray-400 hover:text-white px-3 py-1.5 rounded-lg" style={{background:"#0E1525",border:"1px solid #1A2744"}}>← BACK</button>
        <span className={`px-2.5 py-1 rounded-lg mono text-sm font-bold c${cat[0]}`}>{TYPE_SHORT[inc.type]||inc.type}</span>
        <div><div className="text-lg font-bold text-white">{formatAddress(inc.addr)}</div><div className="mono text-[11px] text-gray-400">{TYPE_LABELS[inc.type]||inc.type} · {BUREAU_NAMES[inc.agency]||inc.agency} · {inc.units.length} UNITS</div></div>
      </div>
    ...
    <div className="flex-1 overflow-auto p-4 space-y-4">
      ...
      <div className="pn p-4 space-y-3" style={{borderLeft:`3px solid ${cc}`}}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="p-3 rounded-lg" style={{background:"#0A0F1A"}}><div className="mono text-[8px] text-gray-500 tracking-widest mb-1">BUREAU LOAD</div><PolySpark d={bureauHist} h={30} threshold={6}/></div>
          <div className="p-3 rounded-lg" style={{background:"#0A0F1A"}}><div className="mono text-[8px] text-gray-500 tracking-widest mb-1">CITYWIDE ACTIVITY</div><PolySpark d={history} h={30} threshold={10}/></div>
        </div>
        <div className="space-y-1.5">{aiLines.map((l,i)=><div key={i} className="flex items-start gap-2"><span className="mono text-[10px] mt-0.5" style={{color:i===0?cc:"#5B9BFA"}}>▸</span><span className="text-[12px] leading-relaxed" style={{color:i===0?"#E8ECF2":"#7A879C"}}>{l}</span></div>)}</div>
      </div>
      {/* Incident data + Maps link */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {[["TYPE",TYPE_LABELS[inc.type]||inc.type],["CALL TIME",formatTime(inc.time)],["BUREAU",BUREAU_NAMES[inc.agency]||inc.agency],["UNITS",String(inc.units.length)],["LATITUDE",inc.lat||"—"],["LONGITUDE",inc.lng||"—"]].map(([l,v])=>
          <div key={l} className="p-3 rounded-lg" style={{background:"#0A0F1A"}}><div className="mono text-[8px] text-gray-500 tracking-widest">{l}</div><div className="mono text-sm text-gray-200 mt-1">{v}</div></div>)}
      </div>
      {inc.lat&&inc.lng&&<a href={`https://maps.apple.com/?q=${inc.lat},${inc.lng}`} target="_blank" rel="noopener" className="inline-flex items-center gap-2 mono text-[11px] font-bold py-2 px-4 rounded-lg" style={{background:"rgba(45,127,249,.08)",color:"#2D7FF9",border:"1px solid rgba(45,127,249,.12)"}}>📍 OPEN IN MAPS →</a>}
      {/* Units grouped by status */}
      <div className="space-y-3">{bs.map(([st,us])=><div key={st}><div className="flex items-center gap-2 mb-2"><div className="w-2.5 h-2.5 rounded-full" style={{background:STATUS_COLORS[st]||"#3D4D66"}}/><span className="mono text-[11px] font-bold" style={{color:STATUS_COLORS[st]||"#3D4D66"}}>{STATUS_LABELS[st]||st} — {us.length}</span></div><div className="flex flex-wrap gap-2 ml-5">{us.map((u,i)=><div key={u.id+i} className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{background:`${STATUS_COLORS[u.status]||"#3D4D66"}08`,border:`1px solid ${STATUS_COLORS[u.status]||"#3D4D66"}18`}}><span className="mono text-base font-bold" style={{color:STATUS_COLORS[u.status]||"#3D4D66"}}>{u.id}</span><span className="text-[9px] text-gray-500">{getUnitType(u.id)}</span></div>)}</div></div>)}</div>
    </div>
  </div>;
}
...
export default function App(){
  ...
  return<div className="flex flex-col" style={{background:"#04070D",minHeight:"100vh"}}>
    ...
    <div className="flex-1 overflow-auto">
      ...
      <div className="p-3 pb-2"><div className="pn overflow-hidden" style={{borderTop:"2px solid #2D7FF9"}}>
        <div className="flex items-center justify-between px-4 py-2" style={{background:"linear-gradient(90deg,rgba(45,127,249,.05),transparent)",borderBottom:"1px solid #1A2744"}}>
          ...
          <span className="mono text-[8px] px-2 py-0.5 rounded" style={{background:"rgba(45,127,249,.06)",color:"#5B9BFA",border:"1px solid rgba(45,127,249,.1)"}}>LIVE · PROCESSING</span>
        </div>
        <div className="p-4"><PolySpark d={history} h={56} threshold={10} label="CITYWIDE INCIDENT LOAD" val={incidents.length}/></div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 px-4 pb-3">
          {[
            {l:"EMS CALLS",v:st.c.ems,c:st.c.ems>5?"#FF3B5C":"#00E08E"},
            {l:"FIRE/TC",v:st.c.fire+st.c.tc,c:st.c.fire>2?"#FF3B5C":"#FFB020"},
            {l:"UNITS DEPLOYED",v:st.tu,c:st.tu>40?"#FF3B5C":"#00E08E"},
            {l:"AVG WALL TIME",v:`${avgW}m`,c:avgW>30?"#FF3B5C":avgW>20?"#FFB020":"#00E08E"},
          ].map(s=><div key={s.l} className="flex items-center justify-between p-2 rounded-lg" style={{background:"#0A0F1A"}}><span className="mono text-[8px] text-gray-500 tracking-wider">{s.l}</span><span className="mono text-lg font-bold" style={{color:s.c}}>{s.v}</span></div>)}
        </div>
      </div></div>

      {/* Hospitals + RA Transports */}
      <div className="px-3 pb-2 grid grid-cols-2 lg:grid-cols-4 gap-2">
        {hosp.map(h=><div key={h.short} className="pn px-3 py-2" style={{borderLeft:`3px solid ${sc(h.status)}`}}>
      ...
      {/* RA Transport strip */}
      <div className="px-3 pb-2"><div className="pn px-4 py-2 flex items-center gap-4 flex-wrap" style={{borderLeft:"3px solid #A855F7"}}>
        ...
        <div className="hidden lg:flex items-center gap-3 ml-auto">{atW.concat(enR).slice(0,5).map(t=><div key={t.unit} className="flex items-center gap-1.5"><span className="mono text-[10px] font-bold" style={{color:t.level==="ALS"?"#FF6B35":"#2D7FF9"}}>{t.unit}</span><span className="mono text-[9px] text-gray-500">{t.hospital}</span>{t.wallTime>0&&<span className="mono text-[10px] font-bold" style={{color:t.wallTime>30?"#FF3B5C":"#00E08E"}}>{t.wallTime}m</span>}</div>)}</div>
      </div></div>

      {/* Main split: Incidents | Heat Maps */}
      <div className="px-3 pb-2 flex gap-2" style={{minHeight:380}}>
    ...
    <footer className="flex-shrink-0 px-4 py-1 flex items-center justify-between" style={{borderTop:"1px solid #1A2744"}}><span className={`mono text-[9px] font-bold ${live?"text-emerald-400":"text-amber-400"}`}>● FIREDASH v14.0</span><span className="mono text-[9px] text-gray-600">APOT SOLUTIONS, INC.</span></footer>
  </div>;
}
